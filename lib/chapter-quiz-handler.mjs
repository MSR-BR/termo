  ...
  if (attemptType === "full_quiz" && mastered) {
    ...
    if (nextQuiz) {
      return {
        ...
        reason: `Voce chegou a ${result.score}% neste simulado; 80% ja libera o proximo desafio.`
      };
    }
    ...
    return {
      ...
      reason: `Voce chegou a ${result.score}% e consolidou o capitulo. Ainda nao ha proximo simulado publicado nesta fase.`
    };
  }
...
export async function handleChapterQuizRequest({
  ...
  if (!quiz) {
    return jsonResponse(404, {
      error: "Quiz nao encontrado.",
      details: "Use um chapterId ou quizKey publicado no catalogo versionado."
    });
  }
