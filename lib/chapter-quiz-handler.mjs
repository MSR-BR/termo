...
} from "./gamification-adaptive-v1.mjs";

const ATTEMPT_TYPES = new Set(["full_quiz", "guided_review", "focused_retry", "daily_challenge"]);
const EVENT_TYPE_BY_ATTEMPT = {
  ...
  daily_challenge: "daily_challenge_completed"
};
      ...
      answers: answers.map(function (answer) {
        return {
          ...
          confidence: ["low", "medium", "high"].includes(answer.confidence)
            ? answer.confidence
            : "not_reported",
          helpUsed: answer.helpUsed === true,
          solutionRevealed: answer.solutionRevealed === true
        };
      }),
...
function computeAwardedXp({
  ...
  if (attemptType === "full_quiz") {
    return hasPreviousFullQuiz ? 0 : Math.max(0, Number(quiz.rewardXp || 30));
  }

  if (attemptType === "daily_challenge") {
...
function buildQuizSummary({
  ...
  const isMastered = attemptType === "full_quiz" && mastery?.mastered === true;
...
function buildQuizResponse({
  ...
  return {
    ok: true,
    contractVersion: "termo-gamification-quiz/1.1.0",
    persisted: true,
    userId: user.id,
    quiz: {
...
export async function handleChapterQuizRequest({
  ...
  if (method === "GET" || method === "HEAD") {
    ...
    if (chapterId) {
      ...
      return jsonResponse(generated.status || 502, {
        error: generated.error || "Nao foi possivel preparar o simulado agora.",
        code: generated.errorCode || "quiz_generation_failed",
        requested: {
          chapterId,
          quizKey
        }
  ...
  const previousAttempts = await fetchQuizAttemptRows({
    config,
    userId: user.id,
    attemptType: "",
    limit: 50
  ...
  if (!previousAttempts.ok) {
    return jsonResponse(previousAttempts.status || 500, {
      error: "Nao foi possivel verificar seu historico de quiz.",
      details: previousAttempts.error || "quiz_history_check_failed"
    });
  }
  ...
  const gradedResult = normalizedAttempt.attemptType === "full_quiz"
    ? gradeQuizSubmission({
        quiz,
  ...
  const result = {
    ...gradedResult,
    feedback: enrichFeedbackWithEvidence({
      chapterId: quiz.chapterId,
      feedback: gradedResult.feedback,
      answers: normalizedAttempt.answers
    })
  };
  result.missedFeedback = result.feedback.filter(function (item) {
    return !item.isCorrect;
  });
  const currentAttemptForMastery = {
  ...
  const mastery = evaluateChapterMastery(
    previousAttempts.rows.concat(currentAttemptForMastery),
    quiz.chapterId
  );
  const xpAwarded = computeAwardedXp({
    ...
    result,
    hasPreviousFullQuiz: previousAttempts.rows.some(function (attempt) {
      return attempt.attempt_type === "full_quiz" && attempt.chapter_id === quiz.chapterId;
    })
  });
  const nextAction = normalizedAttempt.attemptType === "daily_challenge"
    ? buildAdaptiveNextAction({ chapterId: quiz.chapterId, missedFeedback: result.missedFeedback, mastery })
    : buildAdaptiveNextAction({ chapterId: quiz.chapterId, missedFeedback: result.missedFeedback, mastery });
  ...
  const eventDay = buildEventDay(completedAt);
  const semanticActivityId = normalizedAttempt.activityId || quiz.quizKey;
  const attemptIdempotencyKey = normalizedAttempt.attemptType === "daily_challenge"
    ? `quiz:daily_challenge:${eventDay}:${semanticActivityId}`
    : `quiz:${normalizedAttempt.attemptType}:${semanticActivityId}:${completedAt}`;
  const ledgerV1Enabled = isGamificationLedgerV1Enabled(env);

  if (ledgerV1Enabled || isGamificationRpcEnabled(env)) {
    const rpcResponse = await supabaseRpcRequest({
      config,
      fn: ledgerV1Enabled
        ? "record_chapter_quiz_attempt_atomic_v1"
        : "record_chapter_quiz_attempt_atomic",
      body: {
        p_user_id: user.id,
        ...(ledgerV1Enabled ? { p_attempt_idempotency_key: attemptIdempotencyKey } : {}),
        ...
        p_event_idempotency_key: attemptIdempotencyKey,
        p_event_payload: {
          quizSummary,
          nextAction,
          policyVersion: ADAPTIVE_POLICY_VERSION,
          activityId: semanticActivityId
        }
      }
    });
