...
} from "./ai-context-package.mjs";
import {
  summarizeMathContractResult,
  validateExerciseMathContract
} from "./math-format-validator.mjs";
import { isAiExerciseEligible } from "./editorial-registry.mjs";
import { isAiExerciseSourceReady } from "./ai-exercise-source-manifest.mjs";
...
const VALIDATION_REVIEW_STATUSES = new Set(["pending", "approved", "rejected", "disabled"]);
const VALIDATION_ERROR_TYPES = new Set([
  "conceitual",
  "latex",
  "alternativa",
  "explicacao",
...
const RETIRED_GEMINI_MODEL_PATTERN = /^gemini-2\.0-(?:flash|flash-lite)(?:-\d+)?$/;
...
function replaceOutsideMathSegments(value = "", transform = (segment) => segment) {
  const tokens = [];
  const masked = String(value || "").replace(/\\\[[\s\S]+?\\\]|\\\([\s\S]+?\\\)/g, (match) => {
    const token = "@@TERMO_MATH_" + tokens.length + "@@";
    tokens.push(match);
    return token;
  });

  return transform(masked).replace(/@@TERMO_MATH_(\d+)@@/g, (_match, index) => tokens[Number(index)] || "");
}
...
function wrapBareLatexExpressions(value = "") {
  ...
  return replaceOutsideMathSegments(value, (segment) =>
    segment.replace(latexRun, (match, lead, candidate) => {
...
function cleanupNestedMathDelimiters(value = "") {
  let text = String(value || "");
  let previous = "";
...
function wrapBareMathTokens(value = "") {
  return replaceOutsideMathSegments(wrapBareLatexExpressions(value), (segment) =>
    segment
      .replace(/(^|[^A-Za-zÀ-ÿ\\])([A-Za-z])_([A-Za-z0-9]+)\b/g, (_match, lead, symbol, subscript) => lead + "\\(" + symbol + "_{" + subscript + "}\\)")
      .replace(/(^|[^A-Za-zÀ-ÿ\\])([A-Za-z])\^([A-Za-z0-9]+)\b/g, (_match, lead, symbol, superscript) => lead + "\\(" + symbol + "^{" + superscript + "}\\)")
  );
}

function wrapBareAssignments(value = "") {
  return replaceOutsideMathSegments(value, (segment) =>
    segment.replace(/([A-Za-z][A-Za-z0-9']*(?:_[A-Za-z0-9]+)?\s*=\s*[^.,;\n]+)(?=[.,;\n]|$)/g, function (_match, snippet) {
...
function normalizeMathText(value = "") {
  const normalized = wrapBareMathTokens(
    wrapBareAssignments(repairGeneratedMathDelimiters(sanitizeGeneratedExerciseText(value))
      .replace(/\r\n?/g, "\n")
    .replace(/\\\\/g, "\\")
  ...
  return cleanupNestedMathDelimiters(collapseOverescapedLatex(separateAdjacentMathAndText(normalized)));
}

function normalizeExerciseTitle(value = "") {
  return sanitizeGeneratedExerciseText(value || "Exercicio")
    .replace(/\r\n?/g, "\n")
    .split("\n")
    .map((line) => cleanupEquation(line))
    .join(" ")
    .replace(/\s{2,}/g, " ")
    .trim() || "Exercicio";
}
...
function normalizeExerciseMathLocally(exercise = {}) {
  return {
    title: normalizeExerciseTitle(exercise.title || "Exercicio"),
...
function normalizeValidationErrorType(value = "") {
  const normalized = String(value || "").trim().toLowerCase();
  return VALIDATION_ERROR_TYPES.has(normalized) ? normalized : "";
}

function validationErrorTypeLabel(value = "") {
  const labels = {
    conceitual: "erro conceitual",
    latex: "erro de formula/equacao",
    alternativa: "alternativa ou gabarito",
    explicacao: "explicacao insuficiente",
    fora_do_conteudo: "fora do conteudo estudado",
    outro: "outro problema"
  };
  return labels[normalizeValidationErrorType(value)] || "";
}

function prefixValidationNote({ status = "", note = "", errorType = "" } = {}) {
  const text = truncateText(note || "", 220);
  const label = validationErrorTypeLabel(errorType);
  if (status !== "sim" || !text || !label) return text || null;
  return truncateText(`[Tipo: ${label}] ${text}`, 220);
}
...
function sanitizeValidationBody(body = {}) {
  return {
    ...
    errorType: normalizeValidationErrorType(body.errorType),
...
function sanitizeAdminValidationBody(body = {}) {
  ...
  return {
    ...
    decision: ["approved", "rejected", "disabled"].includes(decision) ? decision : "",
...
function buildPrompt({
  ...
  const contextPrompt = buildAiExerciseContextPrompt(contextPackage);
  const pageTitleForPrompt = contextPrompt
    ? contextPackage?.teachingSource?.title || contextPackage?.primarySource?.topicTitle || ""
    : pageTitle;
  const pageSubtitleForPrompt = contextPrompt ? "" : pageSubtitle;
  const pageContentForPrompt = contextPrompt ? "" : pageContent;

  return `
Voce e um assistente didatico para um material de Termodinamica.
...
- Quando houver pacote de contexto, respeite a hierarquia de fontes informada no pacote.
- Use o PDF do livro como referencia principal para definicoes, equacoes, sinais e convencoes.
- Use o HTML da pagina para manter o exercicio alinhado ao que o aluno acabou de estudar.
- Use fragmentos relacionados apenas como contexto complementar; nao troque o assunto central da pagina por um topico transversal.
...
- Como a resposta e JSON, cada barra invertida dentro das strings precisa estar duplicada: escreva \\\\( ... \\\\), \\\\[ ... \\\\] e \\\\frac, nao \\( ... \\) cru dentro do JSON.
...
- Nunca deixe formulas em texto ASCII cru, como k_B, sum(...), d/dT, partial, Z_N, F = U - TS ou expressoes com ^ e _ fora de delimitadores LaTeX.
- Responda APENAS em JSON valido.
- Nao use markdown, asteriscos para enfase ou comandos textuais como *dentro*.
- Nao mencione professor, autor, criador do material nem fonte da pagina no enunciado nem na solucao.
...
- Nao deixe atribuicoes como F = U - TS, H = U + PV ou dF = -S dT - P dV fora de delimitadores.
...
const RAW_LATEX_JSON_ESCAPE_PATTERN =
  /(^|[^\\])\\(?=(?:[\[\]()]|(?:bar|begin|beta|cdot|Delta|delta|epsilon|exp|frac|gamma|ge|hat|infty|int|kappa|lambda|le|left|ln|mathit|mathbf|mathrm|mu|nabla|neq|nu|Omega|overline|partial|Phi|phi|rho|rightarrow|right|sigma|sqrt|sum|tau|text|theta|Theta|tilde|times|to|varepsilon)\b))/g;
...
function parseGeminiModelList(env = process.env) {
  const configuredFallbacks = String(env.GEMINI_FALLBACK_MODELS || "")
    .split(/[,\s]+/)
    .map((entry) => entry.trim())
    .filter(Boolean);
  const primaryModel = String(env.GEMINI_MODEL || DEFAULT_GEMINI_MODEL).trim() || DEFAULT_GEMINI_MODEL;
  const seen = new Set();

  return [primaryModel, ...configuredFallbacks, ...DEFAULT_GEMINI_FALLBACK_MODELS].filter((model) => {
    if (!model || RETIRED_GEMINI_MODEL_PATTERN.test(model) || seen.has(model)) return false;
    seen.add(model);
    return true;
  });
}
...
function isTransientGeminiMessage(message = "") {
  return /high demand|overload|temporar|try again|rate limit|resource exhausted|quota|unavailable|timeout|timed out|deadline/i.test(
    String(message || "")
  );
}
...
function shouldRetryGemini(result) {
  ...
  return GEMINI_TRANSIENT_STATUSES.has(Number(result.status)) || isTransientGeminiMessage(extractGeminiErrorMessage(result.data));
}

async function callGeminiJson({ apiKey, model, prompt, temperature = 0.7, timeoutMs = GEMINI_TIMEOUT_MS }) {
  ...
  try {
    ...
    rawBody = await geminiResponse.text();
  } catch (error) {
    ...
    return {
      ...
      data: {
        error: {
          message: aborted
            ? "Tempo de resposta da API Gemini esgotado."
            : "Nao foi possivel chamar a API Gemini."
        }
      }
    };
  } finally {
  ...
  try {
    ...
    data = { raw: rawBody || "Resposta vazia da API Gemini." };
  }
  ...
  const parsedResult = parseGeneratedJson(extracted);
...
async function callGeminiJsonWithFallback({
  ...
  for (const model of models) {
    ...
    for (let attempt = 1; attempt <= maxAttemptsForModel; attempt += 1) {
      ...
      const message = extractGeminiErrorMessage(result.data);
      attempts.push({
        model,
        attempt,
        status: result.status,
        ok: Boolean(result.ok),
        message: truncateText(message || (result.ok ? "ok" : "sem detalhes"), 180)
      });
      ...
      lastResult = result;
      if (!shouldRetryGemini(result)) break;
  ...
  const transientFailure = attempts.some((attempt) =>
    GEMINI_TRANSIENT_STATUSES.has(Number(attempt.status)) || isTransientGeminiMessage(attempt.message)
  );

  return {
    ok: false,
    status: transientFailure ? 503 : (lastResult?.status || 500),
    model: lastResult?.model || models[0] || "",
    attempts,
    data: {
      error: {
        message: transientFailure
          ? "A IA de exercícios está temporariamente ocupada. Tente novamente em alguns instantes."
          : "Não foi possível gerar o exercício com os modelos configurados."
      },
      attempts
    }
  };
...
function buildValidationRecord({
  ...
  return {
    ...
    statement_note: prefixValidationNote({
      status: body.statementStatus,
      note: body.statementNote,
      errorType: body.errorType
    }),
    solution_note: prefixValidationNote({
      status: body.solutionStatus,
      note: body.solutionNote,
      errorType: body.errorType
    }),
    review_status: "pending",
    ai_review_state: "inconclusive",
    ai_review_summary: body.errorType
      ? `Relato enviado para revisao do professor (${validationErrorTypeLabel(body.errorType)}).`
      : "Relato enviado para revisao do professor.",
    ai_correction_advice: "",
...
export async function handleExerciseRequest({
  ...
  if (!isAiExerciseEligible({
    ...
    return {
      status: 403,
      body: {
        error: "Exercícios IA não estão disponíveis para esta seção.",
        reason: "section_not_ai_eligible"
      }
    };
  }

  if (!isAiExerciseSourceReady({
    chapterId: sanitizedBody.chapterId,
    itemId: sanitizedBody.itemId
  })) {
    return {
      status: 503,
      body: {
        error: "As fontes revisadas desta seção não estão disponíveis para geração.",
  ...
  const geminiModels = parseGeminiModelList(env);
  const geminiTimeoutMs = Math.min(
    parsePositiveInteger(env.GEMINI_TIMEOUT_MS, GEMINI_TIMEOUT_MS),
    28000
  );
  const contextPackage = buildAiExerciseContextPackage(sanitizedBody);
  ...
  try {
    ...
    if (!result.ok) {
      return {
        status: result.status,
        body: {
          error: result.data?.error?.message || "Erro retornado pela API Gemini.",
          reason: result.status === 503 ? "gemini_temporarily_unavailable" : "gemini_generation_failed",
          details: {
    ...
    let mathContract = validateExerciseMathContract(normalized);
    let mathRefinementApplied = false;

    if (
      (requiresRemoteMathRefinement(normalized) || !mathContract.ok) &&
      needsMathFormatting(rawExercise)
    ) {
      ...
      mathRefinementApplied = true;
      mathContract = validateExerciseMathContract(normalized);
    }

    if (!mathContract.ok && needsMathFormatting(normalized)) {
      const formatted = await formatExerciseMath({
        apiKey,
        model: result.model,
        language: body?.language || "pt-BR",
    ...
    return {
      ...
      body: {
        ...
        mathContract: summarizeMathContractResult(mathContract),
        mathContractOk: mathContract.ok,
        mathRefinementApplied,
...
export async function handleExerciseValidationRequest({
  ...
  if (
    (sanitizedBody.statementStatus === "sim" || sanitizedBody.solutionStatus === "sim") &&
    !sanitizedBody.errorType
  ) {
    return {
      status: 400,
      body: { error: "Escolha o tipo principal de problema encontrado no exercicio." }
    };
  }
...
function buildAdminReviewUpdate({ report = {}, reviewerUser = null, decision = "", adminNote = "" }) {
  ...
  if (decision === "disabled") {
    return {
      ...
      ai_review_summary: "Memoria de correcao desativada pelo professor.",
      ai_correction_advice: truncateText(adminNote || report.ai_correction_advice || notes || "", 220),
      avoid_propagation: false
    };
  }
  ...
  return {
    ...
    body: {
      message: reviewBody.decision === "approved"
        ...
        : reviewBody.decision === "disabled"
          ? "Memoria desativada. O relato continua no historico, mas nao entra nas proximas geracoes."
          : "Validacao rejeitada.",
