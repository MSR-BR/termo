...
function wrapBareMathTokens(value = "") {
  return replaceOutsideMathSegments(value, (segment) =>
    segment
      .replace(/(^|[^A-Za-z\\])([A-Za-z])_([A-Za-z0-9]+)\b/g, (_match, lead, symbol, subscript) => lead + "\\(" + symbol + "_{" + subscript + "}\\)")
      .replace(/(^|[^A-Za-z\\])([A-Za-z])\^([A-Za-z0-9]+)\b/g, (_match, lead, symbol, superscript) => lead + "\\(" + symbol + "^{" + superscript + "}\\)")
  );
}
