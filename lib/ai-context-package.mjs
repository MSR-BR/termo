...
} from "./ai-exercise-source-manifest.mjs";
...
function buildSourceReferences(contextPackage = {}) {
  ...
  if (contextPackage.sourceManifest) {
    references.push({
      type: "source_manifest",
      role: "governance",
      sectionId: contextPackage.sectionId,
      reviewStatus: contextPackage.sourceManifest.reviewStatus,
      eligible: contextPackage.sourceManifest.eligible,
      bookSourceFile: contextPackage.sourceManifest.bookSourceFile,
      pageStart: contextPackage.sourceManifest.pageStart,
      pageEnd: contextPackage.sourceManifest.pageEnd,
      primaryTopic: contextPackage.sourceManifest.primaryTopic
    });
  }
...
export function buildAiExerciseContextPackage({
  ...
  const manifestEntry = getAiExerciseSourceManifestEntry({
    chapterId: normalizedChapterId,
    itemId: normalizedItemId
  });
  const sourceManifestReady = isAiExerciseSourceReady({
    chapterId: normalizedChapterId,
    itemId: normalizedItemId
  });
  ...
  if (!sourceManifestReady) {
    return {
      version: "ai-03-context-package-v2",
      sectionId,
      difficulty,
      sourceManifest: manifestEntry ? {
        reviewStatus: manifestEntry.reviewStatus,
        eligible: manifestEntry.eligible === true,
        bookSourceFile: manifestEntry.bookSource?.fileName || "",
        pageStart: manifestEntry.bookSource?.pageStart || 0,
        pageEnd: manifestEntry.bookSource?.pageEnd || 0,
        primaryTopic: manifestEntry.thematicReference?.primaryTopic || ""
      } : null,
      primarySource: null,
      teachingSource: null,
      fallbackSource: null,
      ...
      meta: {
        ...
        sourceReviewStatus: manifestEntry?.reviewStatus || "missing",
        hasPrimaryBookSource: false,
        hasTeachingSource: false,
        relatedFragmentCount: 0,
        advancedSupportFragmentCount: 0,
        topicIndexFound: false
      }
    };
  }
  const relativePagePath = normalizeRelativePagePath(manifestEntry.url);
  ...
  const teachingContent = localTeachingContent;
  ...
  const contextPackage = {
    ...
    sourceManifest: {
      ...
      eligible: manifestEntry.eligible === true,
      bookSourceFile: manifestEntry.bookSource.fileName,
      pageStart: manifestEntry.bookSource.pageStart,
      pageEnd: manifestEntry.bookSource.pageEnd,
      primaryTopic: manifestEntry.thematicReference.primaryTopic
    },
  ...
  return {
    ...
    meta: {
      ...
      sourceReviewStatus: manifestEntry.reviewStatus,
...
export function buildAiExerciseContextPrompt(contextPackage = {}) {
  ...
  if (contextPackage.sourceManifest) {
    const sourceManifest = contextPackage.sourceManifest;
    lines.push(
      "Manifesto de fontes aprovado:",
      `- secao: ${contextPackage.sectionId}`,
      `- estado_de_revisao: ${sourceManifest.reviewStatus}`,
      `- arquivo_pdf: ${sourceManifest.bookSourceFile}`,
      `- paginas: ${sourceManifest.pageStart}${sourceManifest.pageEnd !== sourceManifest.pageStart ? `-${sourceManifest.pageEnd}` : ""}`,
      `- topico_principal: ${sourceManifest.primaryTopic}`,
      ""
    );
  }
