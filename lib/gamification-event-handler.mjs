...
function buildEventResponse({
  ...
  return {
    ok: true,
    contractVersion: "termo-gamification-event/1.0.0",
    persisted,
    deduped,
    awarded,
    reason,
...
export async function handleGamificationEventRequest({
  ...
  if (ledgerV1Enabled || isGamificationRpcEnabled(env)) {
    const rpcResponse = await supabaseRpcRequest({
      config,
      fn: ledgerV1Enabled
        ? "apply_gamification_event_atomic_v1"
        : "apply_gamification_event_atomic",
      body: {
        p_user_id: user.id,
        p_event_type: normalizedEvent.eventType,
        p_idempotency_key: normalizedEvent.idempotencyKey,
        p_event_day: eventDay,
        ...(ledgerV1Enabled ? { p_occurred_at: normalizedEvent.occurredAt || null } : {}),
