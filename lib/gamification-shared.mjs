const DEFAULT_ENABLED_QUIZ_CHAPTER_IDS = ["01", "02", "03", "04", "06"];
...
export const GAMIFICATION_PROFILE_CONTRACT_VERSION = "termo-gamification-profile/1.1.0";
export const GAMIFICATION_POLICY_VERSION = "termo-gamification-policy/1.1.0";
...
export function isGamificationLedgerV1Enabled(env = process.env) {
  return TRUE_ENV_VALUES.has(String(env.TERMO_GAMIFICATION_LEDGER_V1 || "").trim().toLowerCase());
}
...
export async function fetchEventByIdempotencyKey({ config, userId, idempotencyKey }) {
  const response = await supabaseRestRequest({
    config,
...
export async function fetchQuizAttemptRows({
  ...
  const response = await supabaseRestRequest({
    ...
    path: "chapter_quiz_attempts",
    params: {
      select: "id,idempotency_key,quiz_key,chapter_id,attempt_type,score,correct_count,question_count,xp_awarded,answers,feedback,started_at,completed_at",
      user_id: `eq.${userId}`,
      ...(attemptType ? { attempt_type: `eq.${attemptType}` } : {}),
      order: "completed_at.desc",
...
export function buildChapterProgressFromAttempts(attemptRows = []) {
  ...
  attemptRows.forEach(function (attempt) {
    ...
    if (attempt.attempt_type !== "full_quiz") return;
    ...
    const existing = byChapter.get(chapterId) || {
      ...
      mastered: false,
      masteryEvidence: {
        qualifyingRetrievals: 0,
        distinctSessions: 0,
        distinctRepresentations: 0
      },
      excellent: false,
      status: "not_started",
    ...
    const qualifying = attemptRows.filter(function (candidate) {
      if (candidate.attempt_type !== "full_quiz" || String(candidate.chapter_id || "") !== chapterId) return false;
      if (Number(candidate.score || 0) < CHAPTER_MASTERY_SCORE) return false;
      const feedback = Array.isArray(candidate.feedback) ? candidate.feedback : [];
      const correctFeedback = feedback.filter(function (item) { return item.isCorrect === true; });
      return correctFeedback.length > 0 && correctFeedback.every(function (item) { return item.masteryEligible === true; });
    });
    const distinctSessions = new Set(qualifying.map(function (candidate) {
      return String(candidate.completed_at || "").slice(0, 10);
    }).filter(Boolean));
    const distinctRepresentations = new Set(qualifying.map(function (candidate) {
      return String(candidate.quiz_key || "");
    }).filter(Boolean));
    existing.masteryEvidence = {
      qualifyingRetrievals: qualifying.length,
      distinctSessions: distinctSessions.size,
      distinctRepresentations: distinctRepresentations.size
    };
    existing.mastered = qualifying.length >= 2 && distinctSessions.size >= 2 && distinctRepresentations.size >= 2;
    existing.excellent = existing.bestScore >= CHAPTER_EXCELLENCE_SCORE;
    existing.status = existing.mastered ? "mastered" : "review";
    existing.statusLabel = existing.mastered ? "Dominio sustentado" : "Evidencia em construcao";
    existing.statusHint = existing.mastered
      ? "Duas recuperacoes independentes em dias e formas distintas."
      : "Um resultado isolado nao declara dominio; revise e recupere novamente em outro dia.";
...
export function mapProfileRowToJourneyProfile(user = {}, row = {}, options = {}) {
  ...
  const storedMasteryUsesAdaptivePolicy = String(row.policy_version || "") === GAMIFICATION_POLICY_VERSION;
  const summary = {
    ...
    chaptersMasteredCount: Math.max(
      masteredFromAttempts,
      storedMasteryUsesAdaptivePolicy ? Number(row.chapters_mastered_count || 0) : 0
    )
  };
  ...
  return {
    ok: true,
    contractVersion: GAMIFICATION_PROFILE_CONTRACT_VERSION,
    snapshot: {
      version: String(row.projection_version || "legacy-phase-1c"),
      policyVersion: String(row.policy_version || GAMIFICATION_POLICY_VERSION),
      ledgerCursor: Math.max(0, Number(row.ledger_cursor || 0)),
    ...
    source: {
      mode: row.projection_version ? "ledger_projection" : "legacy_live_profile",
      details: row.projection_version
        ? "Snapshot autoritativo derivado do ledger e das projeções versionadas."
        : "Dados legados lidos de gamification_profiles antes do rollout do ledger v1."
    }
  };
}
...
function buildUnlockedBadges({
  ...
  if (
    ...
    maybeUnlock({
      ...
      description: "Voce confirmou o desempenho em recuperacoes independentes, em dias e formas diferentes.",
      awardedAt: new Date().toISOString()
    });
  }
...
export function applyEventToProfile({
  ...
  if (payload.nextAction && typeof payload.nextAction === "object") {
    ...
    next.active_missions_json = [{
      key: `adaptive:${payload.nextAction.type || "continue"}:${chapterId || "general"}`,
      title: payload.nextAction.label || "Continuar estudo",
      description: payload.nextAction.reason || "Proximo passo sugerido a partir das evidencias recentes.",
      href: payload.nextAction.href || "index.html?view=chapters",
      source: payload.nextAction.source || "Evidencias recentes do percurso",
      estimatedMinutes: Math.max(0, Number(payload.nextAction.estimatedMinutes || 0)),
      optional: true,
