...
function normalizeRow(row = {}) {
  return {
    ...
    emailUpdatesOptedIn: row.email_updates_opted_in === true,
    emailUpdatesOptedInAt: row.email_updates_opted_in_at || "",
    emailUpdatesOptedOutAt: row.email_updates_opted_out_at || "",
    emailUpdatesPausedUntil: row.email_updates_paused_until || ""
  };
}
...
export async function handleLegalPreferencesRequest({ method, headers = {}, body, env = process.env }) {
  ...
  const emailPauseDays = Number(input.emailPauseDays || 0);
  const pauseRequested = [30, 90].includes(emailPauseDays);
  const resumeRequested = input.resumeEmailUpdates === true;
  if (!acceptDocuments && typeof emailUpdatesOptedIn !== "boolean" && !pauseRequested && !resumeRequested) {
  ...
  if (typeof emailUpdatesOptedIn === "boolean") {
    ...
    patch.email_updates_paused_until = null;
  }
  if (pauseRequested) {
    patch.email_updates_paused_until = new Date(Date.now() + emailPauseDays * 86400000).toISOString();
  }
  if (resumeRequested) {
    patch.email_updates_paused_until = null;
  }
