...
function buildAssetTags(filePath) {
  ...
  return [
    `<link rel="stylesheet" href="${relativeAssetsDir}/termo-share.css">`,
    `<script defer src="${relativeAssetsDir}/termo-share.js"></script>`
  ].join("\n");
}

async function injectIntoFile(filePath) {
  const html = await readFile(filePath, "utf8");
  if (html.includes("termo-share.js") || html.includes("termo-share.css")) {
    return false;
  }
...
console.log(`Arquivos atualizados com compartilhamento: ${updatedCount}`);
